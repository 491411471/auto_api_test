...
@allure.story("续租/取消续租完整流程")
class TestReletCancelReletOrder:
    """续租 → 取消续租 完整流程测试"""

    @allure.title("完整流程：查询账期详情 → 动态计算日期 → 续租 → 取消续租")
    def test_relet_cancel_relet_flow(self, merchant_api_client, db, global_vars):
        yaml_path = os.path.join(
            os.path.dirname(__file__),
            "../../../../data/merchant/scenario/order/relet_cancel_relet_order.yaml"
        )
        ...
        # 步骤3：取消续租
        # ==================================================================
        with allure.step("3. 取消续租"):
            cancel_cfg = config['merchant']['cancel_lease_renewal']
            cancel_body = cancel_cfg['body_template'].copy()
            cancel_body['orderId'] = order_id
            cancel_body['periodList'] = [relet_period]

            allure.attach(
                json.dumps(cancel_body, ensure_ascii=False, indent=2),
                name="步骤3 请求参数",
                attachment_type=allure.attachment_type.JSON
            )

            resp = merchant_api_client.post(cancel_cfg['endpoint'], json=cancel_body)
            assert resp.status_code == cancel_cfg['expected_status'], \
                f"状态码异常: 期望 {cancel_cfg['expected_status']}, 实际 {resp.status_code}"
            resp_json = resp.json()
            ...
            # 执行断言
            for check in cancel_cfg['validate']:
                path = check['path'].lstrip('$').lstrip('.')
                actual = get_value_by_path(resp_json, path)
                validate(actual, check['operator'], check['value'], path)

            logger.info(f"取消续租成功: orderId={order_id}, period={relet_period}")

        logger.info("续租/取消续租流程全部通过")
